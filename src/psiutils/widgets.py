...
from psiutils.constants import COLOURS, PAD
...
def get_styles() -> ttk.Style:
    ...
    style.configure("pale-red-bg.TEntry", fieldbackground=COLOURS["pale-red"])
...
def vertical_scroll_bar(
    ...
    v_scroll = ttk.Scrollbar(master, orient="vertical", command=widget.yview)
...
def status_bar(
    master: tk.Frame, textvariable: tk.StringVar, colour: str = DIM_TEXT
) -> tk.Frame:
...
class VerticalScrolledFrame(ttk.Frame):
    def __init__(self, parent, *args, **kw):
        ...
        self.canvas = tk.Canvas(
            self, bd=0, width=200, height=300, yscrollcommand=vscrollbar.set
        )
